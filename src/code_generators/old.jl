...
function print_code_einsum_notrans(t::Term, symbol::String, fixed)
    # Print python np.einsum code for term t added to symbol. With deltas, only works on 0,2 or 4 dimensional symbol
    # fixed = [] or fixed = ["a", "i"]
    scalar_str = @sprintf "%+12.8f" t.scalar

    function write_extract(t, a, external)
        # t term, a tensor
        write_str = " extract_mat($(get_symbol(a)), \""
        for b in get_indices(a)
            if t.constraints[b] ∉ [VirtualOrbital, OccupiedOrbital]
                throw("Space not supported")
            elseif b ∈ t.sum_indices || sprint(SASQ.print_mo_index, t.constraints, b)[1] in external
                write_str *= t.constraints[b] == VirtualOrbital ? "v" : "o"
            else
                write_str *= t.constraints[b] == VirtualOrbital ? "a" : "i"
            end
    ...
    if length(t.deltas) > 0
        for d in t.deltas
            delta_ind = sprint(SASQ.print_mo_index, t.constraints, d.indices...)
            if 'b' in delta_ind
                fix_b = true
            end
            if 'c' in delta_ind
    ...
    for a in t.tensors
        # if (No indices) or (nothing summed  and 0 external)                  
        if length(get_indices(a)) == 0 || (length(t.sum_indices) == 0 && sum([1 for b in get_indices(a) if sprint(SASQ.print_mo_index, t.constraints, b)[1] in new_ext]) == 0)
            push!(not_summed_tensors, a)
        else
            indices = []
            for b in get_indices(a)
                if b ∈ t.sum_indices || sprint(SASQ.print_mo_index, t.constraints, b)[1] in new_ext
                    push!(indices, sprint(SASQ.print_mo_index, t.constraints, b))
                end
            end
            if join(indices) == new_ext
                push!(not_summed_tensors, a)
    ...
    for a in t.tensors 
        if a ∉ not_summed_tensors
            tensor_str *= ","
            tensor_str *= write_extract(t, a, external)
        end
    end
    ...
    for a in not_summed_tensors
        if length(get_indices(a)) > 0
            notsum_str *= " *"
            notsum_str *= write_extract(t, a, external)
        else
            notsum_str *= " * $(get_symbol(a))"
        end
    end
